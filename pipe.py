...
def size_SRAM():
    ...
    for i in range(len(weights)):
        ...
        if(t > 4*1024*1024*8):
            total.append(t//2)
            total.append(t - t//2)  #Assuming all data is in the DRAM, everytime we fetch the data from DRAM
            if(i < len(weights)-1):
                feature_2.append(feature[i+1]//2) #When we only fetch half of that layer, we will get half of the output. When we store the data while processing that layer, we will only store half. 
                a=feature[i+1]-feature[i+1]//2
                feature_2.append(a)
        else:
            feature_2.append(feature[i])
            total.append(t)
    feature_2.append(feature[len(feature)-1])
    output_number = len(feature_2)
    max_sum = max(total)
    ...
    return (4)  # for the moment we take SRAM as 8 MB by adding 2.7 MB of additional storage space in on-chip SRAM
...
def length_SRAM_CHUNKS_FROM_DRAM(index_input):
    ...
    generate_chunks_for_sram(total_data_to_transfer,index_input)
    total_chunks = 1
...
'''Non-Operation'''
def generate_chunks_for_sram(size,index_input):
    ...
    total_chunks = 1
    chunk_size = feature_2[index_input]
...
def time_SRAM_DRAM(index_input):
    ...
    Output_data_size_SRAM = feature_2[index_input+1]
...
'''Operation'''#compute all atomic operations of one chunk in CBUF from SRAM
def total_time_per_SRAM_chunk(direction,resnet_flag,cached_for_resnet,index_sram_chunk,index_cbuf_chunk,index_input):
    ...
    logging.info("Total_time_to_transfer : {}".format(total_time_SRAM_chunk))
...
def total_time_per_layer(direction,resnet_flag, index_input,cached_for_resnet):
    ...
    for i in range(total_chunks_from_DRAM_SRAM):   #different chunks stored in SRAM from DRAM
        ...
        for j in range(total_chunks_from_SRAM_CBUF_per_SRAM_chunk[i]): # different chunks stored in CBUF from SRAM
            logging.info("Computing for chunk number {0} in CBUF from SRAM. Top level chunk number in SRAM from DRAM is {1}".format(j,i))
            time , data = total_time_per_SRAM_chunk(direction,resnet_flag,cached_for_resnet,i,j,index_input)
...
def total_inference_time():
    ...
    global SRAM_CHUNKS_FROM_DRAM,CBUF_CHUNKS_FROM_SRAM,Clock_freq, previous_output_in_SRAM, feature_2

    layers = len(feature_2)-1 #remove the final output
    ...
    for layer in range(layers):
        if (layer%2 == 0 and layer > 4 and layer < 51):
            resnet_flag = 1
            cached_for_resnet = feature_2[layer-2]
            logging.info("LAYER NUMBER : {}".format(layer + 1))
            logging.info("Layer size: {}".format(weights[layer]+ feature[layer]))
            
            logging.info("Resnet flag: {0} , Cached output: {1}".format(resnet_flag, cached_for_resnet))
            ...
            time_inference += layer_time
            logging.info("Completed Layer..")
        logging.info("layer_time:{}".format(layer_time))
    ...
    total_inference_time = time_inference # assuming cpu and nvdla run at same clock frequency
