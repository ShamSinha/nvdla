...
# logging 
logging.basicConfig(filename="./pipe-2.log", 
...
def size_SRAM():
    ...
    for i in range(len(weights)):
        ...
        total.append(t)
        if t> 2*1024*1024*8:
            length = math.ceil(t / (2*1024*1024*8))             #按照总的(Input_feature_map + weights)来分
            flag_i_a.append(len(feature_2)) #记录开始的那个点
            flag_i.append(length)  #记录长度
            for a in range(length-1):
                feature_2.append(feature[i+1]/length)      #这个是强调输出的，因为第i输入被拆分成length份，意味着第i+1输出被拆分成length份
                total1.append(2*1024*1024*8)
            feature_2.append(feature[i+1] - (feature[i+1]/length)*(length-1)) #the remaining data
            total1.append(t - 2*1024*1024*8*(length -1))
            print(length)
        else:
            flag_i.append(0)
            flag_i_a.append(0)
            feature_2.append(feature[i])
            total1.append(t)
    max_sum = max(total)
    denominator=1024*1024*8
    max_sum = max_sum / denominator
    #return (math.ceil(round(max_sum,3)))
    print(total)
    return (2)  #change this part to make the SRAM as a specific smaller space: 4MB ******************CHANGED PART******
...
SRAM_size = size_SRAM() #size_SRAM()  #MB  SRAM_size only used for defining the SPACE size of SRAM #why did not use the return value from size_SRAM()
SRAM_size_in_bits = SRAM_size*1024*1024*8   # total size of SRAM 
#chunk_size_DRAM_SRAM = 0.5*1024*1024*8      # Remaining size(free space) of SRAM = SRAM_size_in_bits - previous_output_in_SRAM or total_data_to_transfer (<SRAM_size_in_bits)
chunk_size_DRAM_SRAM = SRAM_size*1024*1024*8      # Remaining size(free space) of SRAM = SRAM_size_in_bits - previous_output_in_SRAM or total_data_to_transfer (<SRAM_size_in_bits)
...
def time_DRAM_SRAM(direction, index_input,index_sram_chunk):
    ...
    global GDDR6_reading_time, weights, feature,total, SRAM_writing_time, delay_bdma, SRAM_CHUNKS_FROM_DRAM,GDDR6_clock_freq,SRAM_clock_freq
    ...
    total_data_to_transfer = total[index_input] # total data to be transferred from DRAM -> SRAM 
...
def length_SRAM_CHUNKS_FROM_DRAM(index_input):
    ...
    '''if(index_input == 0):
        ...
        total_data_to_transfer = total[index_input]
...
def generate_chunks_for_sram(size):
    ...
    remaining_SRAM_space = SRAM_size_in_bits
...
def can_sram_fit(total_size):
    ...
    global SRAM_size_in_bits, SRAM_CHUNKS_FROM_DRAM
    if(SRAM_size_in_bits >= total_size):                            #****************CHENGED PART*************
        logger.info("SRAM_CHUNKS_FROM_DRAM holds the total_size of feature + weights : {} MB".format(total_size))
        SRAM_CHUNKS_FROM_DRAM.append(total_size)
        return True
    return False
...
'''
x=4 #how many CMAC cells
size_atomic_op = 8*(64*1*1 + 64*1*1*x*2)
...
'''Operation'''
def time_SRAM_DRAM(index_input):
    ...
    Output_data_size_SRAM = feature[index_input+1]
...
'''Operation'''
def total_time_per_SRAM_chunk(direction,resnet_flag,flag_fc,cached_for_resnet,index_sram_chunk,index_cbuf_chunk,index_input):
    ...
    copy_time = time_SRAM_DRAM(index_input)/SRAM_clock_freq
...
def total_time_per_layer(direction,resnet_flag, flag_fc,index_input,cached_for_resnet):
    ...
    global previous_output_in_SRAM, weights , feature, feature_2, GDDR6_clock_freq
    total_time_layer = 0
    ...
    for i in range(total_chunks_from_DRAM_SRAM):
        ...
        for j in range(total_chunks_from_SRAM_CBUF_per_SRAM_chunk[i]): # all the sram chunks and the associated sub-chunks for each of the sram chunks must be computed per layer    
            ...
            time , data = total_time_per_SRAM_chunk(direction,resnet_flag,flag_fc,cached_for_resnet,i,j,index_input)
            total_time_layer += time
            #previous_output_in_SRAM += data
    previous_output_in_SRAM = feature_2[index_input ] 
...
def time_SRAM_PDP_back(index_input):
    ...
    ''' check this logic '''
    global feature, weights, total, delay_PDP, Clock_freq
    RDMA_size = 8192 #bits
...
def time_SRAM_RUBIK(index_input):
    ...
    global feature, feature_2, Clock_freq
    RDMA_size = 8192 # bits
...
def total_inference_time_rcnn():
    '''calculate the total inference time 
       ...
       Other operations include : 
          ...
          How will this work? 
            ...
            first FC layer. 
    '''
    global SRAM_CHUNKS_FROM_DRAM, CBUF_CHUNKS_FROM_SRAM, total, weights
    ...
    for i in range(len(weights)):
        if (i%3 == 0 and i<=6):
            ...
            SRAM_CHUNKS_FROM_DRAM = []
            CBUF_CHUNKS_FROM_SRAM = []
        logging.info("layer_time: {}".format(total_time_per_layer))
    print(len(total))
